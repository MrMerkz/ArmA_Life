...
if (_art == "init") then
{
		...
		if ((typeName _this) == "OBJECT") then
		{
			if (not(isNull _this)) then
			{
				if (((getPos _this) select 2) < 5) then
				{
					...
					if (player distance _this < 25) then // All players within 25m die instantly
					{
						player setdamage 1;
					};
				};
	...
	INV_BombIdExists =
	{
		...
		{
			if (((INV_ServerBombArray select _i) select 0) == _this) exitWith
			{
				_result = true;
			};
		};
		...
		for [{_i=0}, {_i < (count INV_ServerBombArray)}, {_i=_i+1}] do
		{
			...
			switch (_name) do
			{
				...
				case "aktivierungsbombe":
				{
					if (!(_settings select 0)) then
					{
						_bombhandler = _vehicle addEventHandler ["engine", {
		...
		for [{_i=0}, {_i < (count INV_LocalBombArray)}, {_i=_i+1}] do
		{
			...
			if ( (vehicle player == _vehicle) and ((count _actionArr) == 0) ) then
			{
				...
				switch _item do
				{
					case "fernzuenderbombe":
					{
						_a1 = _v addAction [localize "STRS_inv_item_vehiclebomb_action_deactivate", "noscript.sqf", format["[""config"", ""defuse"", ""%1"", ""%2""] execVM ""bombs.sqf"";", _item, _id ]];
						_actionArr = _actionArr + [_a1];
					};
					case "zeitzuenderbombe":
					{
						_a1 = _v addAction [localize "STRS_inv_item_vehiclebomb_action_deactivate", "noscript.sqf",  format["[""config"", ""defuse"", ""%1"", ""%2""] execVM ""bombs.sqf"";", _item, _id ]];_a2 = _v addAction [localize "STRS_inv_item_vehiclebomb_action_changetimer", "noscript.sqf", format["[""config"", ""Timer"",  ""%1"", ""%2""] execVM ""bombs.sqf"";", _item, _id ]];
			...
			if (vehicle player != _vehicle) then
			{
				for [{_j=0}, {_j < (count _actionArr)}, {_j=_j+1}] do
				{
					_vehicle removeAction (_actionArr select _j);
				};
				_actionArr = [];
			};
