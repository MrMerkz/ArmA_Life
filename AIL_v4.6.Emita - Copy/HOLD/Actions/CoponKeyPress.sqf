...

{
	...
	if(INV_shortcuts)then{INV_shortcuts=false; titletext["keys off", "PLAIN DOWN"];[] execVM "actions\actionsRemove.sqs"}else{INV_shortcuts=true; titletext["keys on", "PLAIN DOWN"];[] execVM "actions\copactions.sqf"};};
	//L key
	case 38: 
	...
	for [{_i=1}, {_i < 3}, {_i=_i+1}] do
		...
		if(!(isnull _atm) and _atm in bankflagarray) exitwith
			...
			_i = 4;
			if(!local_useBankPossible)exitwith{hint "The ATM rejected your card. Maybe the bank was robbed too recently?"};
			[] execVM "atm.sqf"; 
			_handled=true;
