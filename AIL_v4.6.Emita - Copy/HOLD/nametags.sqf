...
for [{_i=1}, {_i < 25}, {_i=_i+2}] do
	...
	_dirV = vectorDir vehicle player;
        ...
        if (vehicle player == player) then {_searchRadius = 3} else {_searchRadius = 0};
	...
	if(count _vcls > 0 and INV_shortcuts)then
		...
		if((_car in INV_VehicleArray) and typeof _car == "LocalBasicWeaponsBox" and !(isnull _car) and vehicle player == player and _distCP < 4) then
	...
	if(count _objs > 0 and INV_shortcuts)then
		...
		if(_tag in shopusearray and _distTP < 5) then
