...
{
	player groupchat "the civ is in a vehicle!"
};

if (!(_civ call ISSE_IsVictim)) exitwith
{
	player groupchat localize "STRS_inventory_checknohandszip"
};

if (animationstate _civ == "CivilSitting") exitwith
{
	player groupchat "Already ziptied"
}; 
if ("ziptie" call INV_GetItemAmount < 1) exitwith
{
	player groupchat "You need a zip tie in your inventory in order to use this function."
};
