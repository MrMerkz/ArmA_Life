...
donatorlevelclassA = ["6847616"];

//if ((getPlayerUID player) in adminlevel1) then {['geld',50000] call INV_AddInvItem};
//if ((getPlayerUID player) in adminlevel2) then {['geld',100000] call INV_AddInvItem};
//if ((getPlayerUID player) in donatorlevel1) then {['geld',10000] call INV_AddInvItem};
//if ((getPlayerUID player) in donatorlevel2) then {['geld',20000] call INV_AddInvItem};
//if ((getPlayerUID player) in donatorlevel3) then {['geld',50000] call INV_AddInvItem};
//if ((getPlayerUID player) in donatorlevelclassA) then {['geld',100000] call INV_AddInvItem};
...
//=================================================================================
INV_smscost		 = 100;
add_civmoney		 = 4500+admincashbonus+donatorcashbonus;
add_copmoney		 = 5000+admincashbonus+donatorcashbonus;
add_workplace		 = 5000;
...
patrolmoneyperkm = 6.0;  // 1 would be equal to $3,000 for 1KM
pmissionactive1 = false;
patrolwaittime1 = false;
patrolmoneyperkm1 = 7.0;  // 1 would be equal to $3,000 for 1KM
//=========government convoy=============
govconvoybonus 		 = 15000;
govconvoythiefbonus	 = 25000;
...
respawntimeinc		 = 5;  //respawn time added every time a civ dies
killedplayerinc          = 10; //Amount of time added per kill for civs
respawnzeit              = 30;  //respawnzeit + extradeadtime = total base respawn also respawn time after a cop dies atleast once      										
DeadWaitSec              = respawnzeit; 							
DeadTimeMax              = 30; //Max time you can get for respawn    								
deadtimebonus            = 0.001;       															
deadtimes                = 0;
...
GesetzAnzahl             = 15;         				
GesetzArray              = ["Always Drive on the RIGHT side of the road","Always Holster weapons in Towns.","Speed limits 60 in town 90 out","","","","","","","","","","",""];
