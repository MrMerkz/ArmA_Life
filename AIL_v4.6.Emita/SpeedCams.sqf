...
_type   = typeof _car;
	...
	if ((player == driver _car) and (_car distance _x < 40) and (_speed >= 99) and !(_car iskindof "Air")) then 
		...
		if(_speed >= 99  and _speed < 110) then {_penalty = 1};
		if(_speed >= 120 and _speed < 130) then {_penalty = 2};
		if(_speed >= 140 and _speed < 150) then {_penalty = 3};
		if(_speed >= 160 and _speed < 170) then {_penalty = 6};
		if(_speed >= 180 		 ) then {_penalty = 7};

		player groupchat format["You have just been flashed by a speed camera Slow down! (Limit: 99, Speed: %1)", round _speed];
