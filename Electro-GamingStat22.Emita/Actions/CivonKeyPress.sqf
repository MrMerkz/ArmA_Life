...

{
	...
	case 25:
	{	
	if(_shift)then{[] execVM "hacker.sqf";};
	};	
	...
	if(INV_shortcuts)then{INV_shortcuts=false; titletext["EG keys off", "PLAIN DOWN"];[] execVM "actions\actionsRemove.sqs"}else{INV_shortcuts=true; titletext["EG keys on", "PLAIN DOWN"];[] execVM "actions\civactions.sqf"};};
	//L key
	case 38: 
