	...
	[] spawn
	{
		...
		R3F_REV_FNCT_creer_marqueur_inconscient =
		{
			if (R3F_REV_CFG_afficher_marqueur && alive player) then
			{
				...
				_marqueur setMarkerType "mil_warning";
		...
		R3F_REV_FNCT_nouvel_inconscient =
		{
			...
			if !(isServer && isDedicated) then 
			{
				if !(isNull _unite) then
				{
					...
					//_id_action = _unite addAction [STR_R3F_REV_action_reanimer, "RPM\Global\STD_NoScript.sqf",[cursorTarget] spawn UltRP_ES_fnc_revive;","cursorTarget isKindOf 'Man' and cursorTarget getVariable 'DEAD' and player distance cursorTarget < 5"],
