if (isNil {INV_SavedVehCop select 0}) then {
player groupChat "You do not have any saved vehicles, currently!";

} else {
_rtv = (INV_SavedVehCop select 0) call inv_getitemname;
_rtv2 = (INV_SavedVehCop select 1) call inv_getitemname;
_rtv3 = (INV_SavedVehCop select 2) call inv_getitemname;
continue = false;
vehicle2Spawn = test;

player removeAction action222;
reTrieve = player addaction [_rtv,"noscript.sqf",'continue = true; vehicle2Spawn = (INV_SavedVehCop select 0);',1,true,true,"",'player distance savepoint2 < 5'];
reTrieve1 = player addaction [_rtv2,"noscript.sqf",'continue = true; vehicle2Spawn = (INV_SavedVehCop select 1);',1,true,true,"",'player distance savepoint2 < 5'];
reTrieve2 = player addaction [_rtv3,"noscript.sqf",'continue = true; vehicle2Spawn = (INV_SavedVehCop select 2);',1,true,true,"",'player distance savepoint2 < 5'];
...
INV_SavedVehCop = INV_SavedVehCop - [vehicle2Spawn];

server globalchat format ["%1",_v];

["INV_SavedVehCop",INV_SavedVehCop] spawn clientsavevar;
