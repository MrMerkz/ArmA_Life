...
// fernzuenderbombe = Remotely Detonated Bomb
// fernzuender = Remote Detonator for Bomb
// zeitzuenderbombe = Time Bomb
// aktivierungsbombe = Car Bomb (Engine Starts up it blows)
// geschwindigkeitsbombe = Car Speed Bomb (Vehicles will blow if speed drops below X)
...
if (_art == "init") then {
		...
		if ((typeName _this) == "OBJECT") then {
			if (not(isNull _this)) then {
				if (((getPos _this) select 2) < 5) then {
					...
					// All players within 25m die instantly
	...
	if (isServer) then {
		["server"] execVM "scripts\carbombs\carbombs.sqf";
	};

	["client"] execVM "scripts\carbombs\carbombs.sqf";
	...
	while {true} do {
		...
		for [{_i=0}, {_i < (count INV_LocalBombArray)}, {_i=_i+1}] do {
			...
			if ( (vehicle player == _vehicle) and ((count _actionArr) == 0) ) then {
				...
				switch _item do {
					case "fernzuenderbombe": {
						_a1 = _v addAction [localize "STRS_inv_item_vehiclebomb_action_deactivate", "scripts\carbombs\carbombsaction.sqf", format["[""config"", ""defuse"", ""%1"", ""%2""] execVM ""scripts\carbombs\carbombs.sqf"";", _item, _id]];
						_actionArr = _actionArr + [_a1];
					};
					case "zeitzuenderbombe": {
						_a1 = _v addAction [localize "STRS_inv_item_vehiclebomb_action_deactivate", "scripts\carbombs\carbombsaction.sqf", format["[""config"", ""defuse"", ""%1"", ""%2""] execVM ""scripts\carbombs\carbombs.sqf"";", _item, _id]];
						_a2 = _v addAction [localize "STRS_inv_item_vehiclebomb_action_changetimer", "scripts\carbombs\carbombsaction.sqf", format["[""config"", ""Timer"",  ""%1"", ""%2""] execVM ""scripts\carbombs\carbombs.sqf"";", _item, _id]];
...
if (_art == "use") then {
	...
	if (_item == "fernzuenderbombe") then {
		["plant", "fernzuenderbombe"] execVM "scripts\carbombs\carbombs.sqf";
	};
	if (_item == "fernzuender") then {
		if (!(createDialog "remotebomb_remote")) exitWith {
			hint "Dialog Error!";
		};
	};
	if (_item == "zeitzuenderbombe") then {
		...
		buttonSetAction [4, "[""plant"", ""zeitzuenderbombe"", sliderPosition 1] execVM ""scripts\carbombs\carbombs.sqf"";closedialog 0;"];
	...
	if (_item == "aktivierungsbombe") then {
		["plant", "aktivierungsbombe"] execVM "scripts\carbombs\carbombs.sqf";
	};
	if (_item == "geschwindigkeitsbombe") then {
		...
		buttonSetAction [4, "[""plant"", ""geschwindigkeitsbombe"", sliderPosition 1, sliderPosition 8] execVM ""scripts\carbombs\carbombs.sqf""; closedialog 0;"];
...
if (_art == "config") then {
	...
	_id   = _this select 3;
	if (!(_id call INV_BombIdExists)) exitWith {
		player groupChat localize "STRS_inv_item_vehiclebomb_notexist,";
	};
	switch _name do {
		case "fernzuenderbombe": {
			if (_art == "defuse") then {
				format["""%1"" call INV_BombDelete", _id] call broadcast;
				[_name, 1] call INV_AddInventoryItem;
		...
		case "zeitzuenderbombe": {
			...
			if (_art == "timer") exitWith {
				...
				_sliderpos = 0;
				ctrlSetText [4, localize "STRS_inv_item_vehiclebomb_changetimer"];
				sliderSetRange [1, zeitbombe_mintime, zeitbombe_maxtime];
				sliderSetSpeed [1, 1, 10];
